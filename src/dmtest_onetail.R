...
run_dm <- function(errors_list, horizon, model_names, benchmark_index) {
  n_models <- length(errors_list)
  p_values <- numeric(n_models)
  statistics <- numeric(n_models)
  
  benchmark_name <- model_names[benchmark_index]
  benchmark_errors <- errors_list[[benchmark_index]]
  
  # Run DM test for each model vs benchmark
  for (i in seq_len(n_models)) {
    if (i != benchmark_index) {
      test_result <- dm.test(benchmark_errors, errors_list[[i]],
                             h = horizon, alternative = "less")
  ...
  results <- data.frame(
    Candidate_Model = model_names,
    Test_Statistic = round(statistics, 4),
    P_Value = round(p_values, 4)
  )
  
  cat("=== Benchmark Model:", benchmark_name, "===\n")
  return(results)
}
...
results_1m_rf_benchmark <- run_dm(models_1m, 1, model_names, benchmark_index = 5)
results_3m_rf_benchmark <- run_dm(models_3m, 3, model_names, benchmark_index = 5)
results_6m_rf_benchmark <- run_dm(models_6m, 6, model_names, benchmark_index = 5)
results_12m_xgb_benchmark <- run_dm(models_12m, 12, model_names, benchmark_index = 6)

print(results_1m_rf_benchmark)
print(results_3m_rf_benchmark)
print(results_6m_rf_benchmark)
print(results_12m_xgb_benchmark)
